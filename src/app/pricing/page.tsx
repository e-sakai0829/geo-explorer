...
import { Check, Sparkles, ShieldCheck, Zap, Building2, HelpCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-10 max-w-6xl mx-auto pb-16">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-2">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-bold border border-blue-200/60">
          <Sparkles className="w-3.5 h-3.5" />
          Transparent Pricing
        </div>
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">
          ビジネス規模に合わせたシンプルな料金プラン
        </h1>
        <p className="text-xs text-slate-500">
          初期費用0円・いつでも解約可能。Stripeによる安全な即時決済に対応
        </p>
      </div>
          ...
          return (
            <div
              ...
              className={`rounded-2xl p-7 flex flex-col justify-between transition-all ${
                p.highlight
                  ? "bg-white border-2 border-blue-600 shadow-lg relative -translate-y-1"
                  : "bg-white border border-slate-200/80 shadow-xs hover:border-slate-300"
              }`}
            >
              {p.highlight && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-[11px] font-bold px-3 py-0.5 rounded-full shadow-sm">
                  人気 No.1 プラン
                </div>
              )}
              ...
              <div>
                ...
                <div className="bg-slate-50 rounded-xl p-3.5 mb-5 space-y-1.5 border border-slate-100 text-xs">
                  <div className="flex justify-between font-bold text-slate-800">
                    <span>調査クレジット:</span>
                    <span className="text-blue-600">{p.credits}</span>
                  </div>
                  <div className="flex justify-between font-bold text-slate-800">
                    <span>記事自動生成:</span>
