  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-200/90 shadow-xs space-y-5">
      ...
      <div className="space-y-3">
        {items.map((item) => {
          const atsDiff = item.afterATS - item.beforeATS;
          const isImproved = atsDiff > 0;
            ...
            >
              ...
              <div className="flex items-center justify-between md:justify-end gap-4 shrink-0 bg-white p-3 rounded-lg border border-slate-200">
                <div className="text-center">
                  ...
                  <div className="text-sm font-black text-slate-600">{item.beforeATS} pt</div>
                </div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-sm font-black text-emerald-600">{item.afterATS} pt</div>
                </div>

                <div className="pl-2 border-l border-slate-100 text-right">
                  <div className={`inline-flex items-center gap-0.5 px-2 py-0.5 rounded text-xs font-black ${
                    isImproved ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700'
                  }`}>
                    {isImproved ? `+${atsDiff} pt 向上 🎉` : `${atsDiff} pt`}
                  </div>
                </div>
              </div>
            </div>
