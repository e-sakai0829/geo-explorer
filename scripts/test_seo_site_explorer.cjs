...
function load(relative, mocks = {}, globals = {}) {
  ...
  process: { env: { DATAFORSEO_API_LOGIN: 'offline-test', DATAFORSEO_API_PASSWORD: 'offline-test' } },
...
const keyword = (word, url, etv) => ({ keyword_data: { keyword: word, keyword_info: { search_volume: 300 }, keyword_properties: { keyword_difficulty: 0 } }, ranked_serp_element: { serp_item: { url, rank_absolute: 1, etv, estimated_paid_traffic_cost: 0 } } });
...
async function main() {
  ...
  let user = null, calls = 0, dbValue = null, fail = false;
  const db = { from() { return this; }, select() { return this; }, eq() { return this; }, abortSignal() { return this; }, maybeSingle: async () => ({ data: dbValue ? { data: dbValue } : null }), upsert() { return { abortSignal: async () => ({ error: { message: 'offline cache' } }) }; } };
  ...
  assert(concurrent.every(r => r.status === 200)); assert.equal(calls, 1); passed++;
